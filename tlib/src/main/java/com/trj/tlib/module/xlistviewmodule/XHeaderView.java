...
//        mArrowImageView = (ImageView) findViewById(R.id.header_arrow);
        mArrowImageView2 = (ImageView) findViewById(R.id.header_arrow2);
...
//        mProgressBar = (ProgressBar) findViewById(R.id.header_progressbar);
    ...
    public void setState(int state) {
        ...
        if (state == STATE_REFRESHING) {
            // show progress
            mArrowImageView2.clearAnimation();
//            mArrowImageView2.setVisibility(View.INVISIBLE);
//            mProgressBar.setVisibility(View.VISIBLE);
        } else {
            // show arrow image
//            mArrowImageView2.setVisibility(View.VISIBLE);
        ...
        switch (state) {
            case STATE_NORMAL:
                if (mState == STATE_READY) {
                    mArrowImageView2.startAnimation(mRotateDownAnim);
                }
                if (mState == STATE_REFRESHING) {
                    mArrowImageView2.clearAnimation();
                }
                mHintTextView.setText(R.string.header_hint_refresh_normal);
                Glide.with(getContext()).load(R.mipmap.jiazai_1).into(mArrowImageView2);
                break;

            case STATE_READY:
                if (mState != STATE_READY) {
                    mArrowImageView2.clearAnimation();
                    mArrowImageView2.startAnimation(mRotateUpAnim);
            ...
            case STATE_REFRESHING:
                ...
                Glide.with(getContext()).load(R.mipmap.jiazai).into(mArrowImageView2);
                break;
